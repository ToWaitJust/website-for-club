...
- **server.js**：HTTP 服务器，监听 `0.0.0.0`，端口从 `DEPLOY_RUN_PORT` 环境变量读取（fallback 5000）；支持 URL 重写（`/ai-learning` → `ai-learning.html`，`/operation` → `operation.html`）；MIME 类型映射完整。
- **index.html**：SPA 风格的首页，含 Landing 视图和主应用视图切换。
- **ai-learning.html**：独立的团队 AI+IT 学习规划展示页。
- **operation.html**：独立的运营方案展示页。
